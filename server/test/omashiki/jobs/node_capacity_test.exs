defmodule Omashiki.Jobs.NodeCapacityTest do
  ...
  *become* one node at a time rather than running two at once. Every assertion
  ...
  alias Omashiki.Jobs
  alias Omashiki.Jobs.{Admission, ExecutionCapacity, Job, JobAttempt}
  alias Omashiki.Repo
  ...
  test "a node with no capacity row claims nothing until boot gives it one", %{
    ...
    become!(root, "node-a")
    {:ok, _, job} = Admission.admit_once(token, request("unbooted"))
  ...
  test "twenty attempts run across two nodes at ten slots each", %{root: root, token: token} do
    ...
    {:ok, _, overflow} = Admission.admit_once(token, request("a-overflow"))
  ...
  test "a manually released slot is not released again by lease expiry", %{
    ...
    become!(root, "node-a")
    {:ok, _, job} = Admission.admit_once(token, request("double-release"))
    ...
    {:ok, _, eleventh} = Admission.admit_once(token, request("eleventh"))
  ...
  test "an attempt with no recorded node releases against the migrated row", %{
    ...
    {:ok, _, job} = Admission.admit_once(token, request("legacy-attempt"))
  ...
  defp claim_concurrently(token, key, count, opts \\ []) do
    jobs =
      Enum.map(1..count, fn n ->
        {:ok, _, job} = Admission.admit_once(token, request("#{key}-#{n}"))
        job
      end)
