defmodule Omashiki.Plugin.OptionSchemaTest do
  ...
  test "validates jcode options", %{plugins: plugins} do
    ...
    assert {:error, {:unknown_options, ["web_search"]}} =
             OptionSchema.validate(manifest, %{"web_search" => true})
    ...
    assert {:error, :invalid_invocation_path} =
             OptionSchema.validate(manifest, %{"invocation_path" => "/etc/prompt.txt"})
  end
  ...
  test "validates claude-code options", %{plugins: plugins} do
    ...
    assert {:error, {:unknown_options, ["extra"]}} =
             OptionSchema.validate(manifest, %{"extra" => true})

    assert {:error, :invalid_timeout} = OptionSchema.validate(manifest, %{"timeout_ms" => 0})

    assert {:error, :invalid_allowed_tools} =
             OptionSchema.validate(manifest, %{"allowed_tools" => ["Bash"]})

    assert {:error, :invalid_invocation_path} =
             OptionSchema.validate(manifest, %{"invocation_path" => "/etc/invocation.json"})
  end
  ...
  test "validates codex options", %{plugins: plugins} do
    ...
    assert {:error, {:unknown_options, ["allowed_tools"]}} =
             OptionSchema.validate(manifest, %{"allowed_tools" => ["Read"]})

    assert {:error, :invalid_timeout} = OptionSchema.validate(manifest, %{"timeout_ms" => 0})

    assert {:error, :invalid_web_search} =
             OptionSchema.validate(manifest, %{"web_search" => "yes"})

    assert {:error, :invalid_model} = OptionSchema.validate(manifest, %{"model" => "--oss"})

    assert {:error, :invalid_reasoning_effort} =
             OptionSchema.validate(manifest, %{"reasoning_effort" => "lowest"})
    ...
    assert {:error, :invalid_credentials_path} =
             OptionSchema.validate(manifest, %{"credentials_path" => "/etc/codex-auth.json"})
  end
  ...
  test "validates pi options", %{plugins: plugins} do
    ...
    assert {:error, {:unknown_options, ["thinking"]}} =
             OptionSchema.validate(manifest, %{"thinking" => "high"})
